...
static bool declaresCoroCleanupIntrinsics(Module &M) {
...
PreservedAnalyses CoroCleanupPass::run(Function &F, FunctionAnalysisManager &AM) {
  auto &M = *F.getParent();
  if (!declaresCoroCleanupIntrinsics(M) ||
      !Lowerer(M).lowerRemainingCoroIntrinsics(F))
    return PreservedAnalyses::all();

  // TODO(modocache): Once again, this is overly pessimistic. In fact, I believe
  // that we can preserve CFG analyses here (since we move the simplifyCFG call
  // out of the lowerer and into the legacy pass).
...
struct CoroCleanupLegacy : FunctionPass {
  ...
  bool runOnFunction(Function &F) override {
    if (L && L->lowerRemainingCoroIntrinsics(F)) {
