...
PreservedAnalyses CoroElidePass::run(Function &F, FunctionAnalysisManager &AM) {
  ...
  // TODO(modocache): There's no reason not to call this function, but it has
  // an assert insisting that the devirt trigger exists in the first place,
  // and we don't want to trip that.
  ...
  // TODO(modocache): This is pessimistic, I think we may preserve some
  // analyses.
  if (Changed)
